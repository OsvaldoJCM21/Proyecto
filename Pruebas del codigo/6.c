...
void consultarPorEmpleado() {
    ...
    printf("Ingrese el nombre del empleado para consultar sus herramientas: ");
    scanf("%s", empleado);

    printf("\n--- Herramientas asignadas a %s ---\n", empleado);
    for (int i = 0; i < total_activos; i++) {
        if (strcmp(inventario[i].empleado, empleado) == 0) {
            encontrado = 1;
            printf("Nombre: %s\n", inventario[i].nombre);
            printf("Tipo: %s\n", inventario[i].tipo);
            printf("Proyecto: %s\n", inventario[i].proyecto);
    ...
    if (!encontrado) {
        printf("No se encontraron herramientas asignadas a %s.\n", empleado);
    }
}

void calcularDepreciacion() {
    for (int i = 0; i < total_activos; i++) {
        float depreciacion_anual = inventario[i].valor_original / inventario[i].vida_util;
        printf("Activo %d - Nombre: %s\n", i + 1, inventario[i].nombre);
        printf("Valor original: %.2f\n", inventario[i].valor_original);
...
void menu() {
    ...
    do {
        ...
        printf("3. Consultar herramientas por empleado\n");
        printf("4. Calcular depreciacion\n"); // Nueva opción
        ...
        switch (opcion) {
            ...
            case 3:
                ...
                break;
            case 4:
                calcularDepreciacion(); // Llamada a la nueva función
                break;
            case 5:
                cambiarClave();
                break;
